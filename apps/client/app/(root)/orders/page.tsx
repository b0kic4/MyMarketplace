...
const Orders = () => {
  ...
  React.useEffect(() => {
    if (userId) {
      ...
      setOrderApiUrl(
        `${process.env.NEXT_PUBLIC_NESTJS_URL}/payments/getOrderByUserId?${url}`,
      );
    }
  }, [userId]);
        ...
        <div className="mt-4">
          {order.purchasedProducts.map((product: any) => (
            <ProductReview
              ...
              product={product}
              orderId={order.id}
              reviews={order.reviews.filter(
                (review: Review) => review.productId === product.productId,
              )}
            />
          ))}
        </div>
...
const ProductReview = ({
  ...
  const mainLogoImage =
    product.product.images.find(
      (image: ProductImage) => image.isLogo === "true",
  ...
  const handleReviewSubmit = async ({
    ...
    if (!response.ok) {
      return toast.error(
        "Error occured when trying to create review for product",
        {
          position: "top-left",
          theme: "dark",
        },
